...
class MainWindowV054(MainWindowV050):
    """V0.5.4: pronúncia com cache local e fallback TTS do Windows."""
    ...
    def _render_dictionary_result(self, result):
        ...
        if result.audio_url:
            self.pronunciation_button.setToolTip(
                "Baixa o áudio para o cache local e reproduz o arquivo local."
            )
        else:
            self.pronunciation_button.setToolTip(
                "Usa uma voz inglesa instalada no Windows como fallback."
            )
    ...
    def _pronunciation_failed(self, message: str):
        QMessageBox.warning(self, "Pronúncia indisponível", message)
